...
void Arcade() {
    ...
    if (x > deadband || y > deadband) {
        ...
        if (x < deadband) {
            ...
            turn = 0.78*(12.7 / (1.0 + exp(-(3.0/4.0)*(LjoyX - 6.0)))) * 10.0 - 3.0; //0.8
            // turn = 0.45*(12.7 / (1.0 + exp(-(3.0/4.0)*(LjoyX - 6.0)))) * 10.0 - 3.0;

        }
        else if (x > 55 && x <= 85) {
            turn = 0.33*(12.7 / (1.0 + exp(-(3.0/4.0)*(LjoyX - 6.0)))) * 10.0 - 3.0;
            // 0.37 initial value
            // turn = 0.4*(12.7 / (1.0 + exp(-(3.0/4.0)*(LjoyX - 6.0)))) * 10.0 - 10.0;
        }
        else {
            turn = 0.28*(12.7 / (1.0 + exp(-(3.0/4.0)*(LjoyX - 6.0)))) * 10.0 - 3.0;
            // 0.32 initial value
